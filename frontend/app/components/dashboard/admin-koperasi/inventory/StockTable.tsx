...
export default function StockTable({ stocks, refreshData }: StockTableProps) {
  ...
  const renderStatusBadge = (item: FertilizerItem) => {
    ...
    switch (computedStatus) {
      ...
      case "menipis":
        return (
          <span className="px-3 py-1 rounded-full text-xs font-bold bg-amber-50 text-amber-500 border border-amber-100">
            Menipis
          </span>
        );
      default:
        return (
          <span className="px-3 py-1 rounded-full text-xs font-bold bg-emerald-50 text-emerald-500 border border-emerald-100">
            Tersedia
          </span>
        );
    }
  ...
  return (
    <div className="flex flex-col h-full space-y-4">
      ...
      {/* TAB NAVIGASI — komponen bersama, dipakai juga di halaman Riwayat Stok */}
      <InventoryTabs />
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-sm font-medium text-zinc-800">
              {stocks.map((item, index) => {
                const packagingSize = item.packaging_size_kg || 50;
                const totalBags = Math.ceil(
                  item.current_stock_kg / packagingSize,
                );
                const pricePerBag = item.price_per_kg * packagingSize;
                const totalStockValue =
                  item.current_stock_kg * item.price_per_kg;
                const isOutOfStock = item.current_stock_kg === 0;

                // Memastikan prefix URL gambar lengkap menuju direktori storage laravel
                const imageUrl = item.image
                  ? `${baseUrl}${item.image}`
                  : "https://placehold.co/40x40?text=Pupuk";

                return (
                  <tr
                    key={item.id}
                    className="border-b border-zinc-200 hover:bg-zinc-50/60 transition-colors"
                  >
                    <td className="py-4 px-4 text-center text-zinc-500 font-normal">
                      {index + 1}
                    </td>
                    <td className="py-4 px-6 font-bold text-zinc-900">
                      <div className="flex items-center gap-3">
                        <img
                          src={imageUrl}
                          alt={item.name}
                          className="w-10 h-10 object-cover rounded-md border border-zinc-100 bg-zinc-50 shrink-0"
                          onError={(e) => {
                    ...
                    <td className="py-4 px-4 text-zinc-900 font-bold uppercase">
                    ...
                    <td className="py-4 px-4 text-zinc-800">
                      {isOutOfStock ? "0 Karung" : `${totalBags} Karung`}
                    </td>
                    <td className="py-4 px-4 text-zinc-800">
                      {isOutOfStock
                        ? "Rp -"
                        : `Rp ${pricePerBag.toLocaleString("id-ID")}`}
                    </td>
                    <td className="py-4 px-4 font-bold text-zinc-900">
                      {isOutOfStock
                        ? "Rp -"
